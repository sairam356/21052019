  ...
  modal:false 
   ...
   this.goodReceipe =[]
    ...
    this.showTables = this.showTables.bind(this);
     this.toggle = this.toggle.bind(this); // new changes
...
//*** new code 
 ...
 changeSelection = selection => {   /// new changes  -- down html is also keep that
       let selection1=null,selection2=[],status =false;
      if(selection.length>1){
         ...
         selection1 = selection[selection.length-1]  
          ...
          _.forEach(this.state.rows,function(data){
              if(data.id == selection1 && data.sex == 'F'){ // status check
                  this.toggle();
                 return; 
              }
          }.bind(this));
      ...
      if(!status){
          _.forEach(this.state.rows,function(data){
                  if(data.id == selection[0] && data.sex == 'F'){ // status check
                      this.toggle();
                     return; 
                  }
              }.bind(this));
